...
inf_dephasing = sim_with_dephasing.gate_fidelity(X_TO_OUR_DARK, use_jax_mc=True)
print(f"Infidelity with dephasing: {inf_dephasing[0]:.6f} +/- {inf_dephasing[1]/np.sqrt(N_MC):.6f}")
print()

# ==================== Position error ====================

# NOTE: sigma_pos_xyz in meters. (70e-9, 70e-9, 130e-9) = (70, 70, 130) nm
...
print(f"Running with position error (sigma_xyz=(70,70,130) nm, {N_MC} MC shots)...")
inf_position = sim_with_position.gate_fidelity(X_TO_OUR_DARK)
print(f"Infidelity with position error: {inf_position[0]:.6f} +/- {inf_position[1]/np.sqrt(N_MC):.6f}")
print()
...
inf_all = sim_with_all_errors.gate_fidelity(X_TO_OUR_DARK)
print(f"Infidelity with ALL errors: {inf_all[0]:.6f} +/- {inf_all[1]/np.sqrt(N_MC):.6f}")
print()
...
print("=" * 65)
print(f"{'Dephasing (130 kHz)':<30} {inf_dephasing[0]:>12.6f} +/- {inf_dephasing[1]/np.sqrt(N_MC):.6f}")
print(f"{'Position (70,70,130 nm)':<30} {inf_position[0]:>12.6f} +/- {inf_position[1]/np.sqrt(N_MC):.6f}")
print(f"{'ALL errors':<30} {inf_all[0]:>12.6f} +/- {inf_all[1]/np.sqrt(N_MC):.6f}")
print("=" * 65)
