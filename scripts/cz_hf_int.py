...
from ryd_gate.blackman import blackman_pulse, blackman_pulse_sqrt
...
class CZ():
    ...
    def __init__(self, mid_state_decay, ryd_decay,
        ...
        self.Delta = 2*np.pi*(9.1)*10**(9)  # E_mid - omega_420
        ...
        self.v_ryd = 2*np.pi*(1199)*10**(6) # Ryd interaction 
        ...
        self.ryd_state_decay_rate = (1/(151.55*10**(-6)))*self.ryd_decay
        self.ryd_garb_decay_rate = (1/(151.55*10**(-6)))*self.ryd_garbage
    ...
    def Opt_phase_mod_bell(self):
        ...
        # init_vals = [2.520450553252122639e+00, 1.148533385060407586e+00, -3.882281266349976612e+00, 
        #              2.381510816431252842e+00, 8.903153423606454453e+00, 3.999115680679551366e-01]
        # optimize over all errs, at zero temp:
        # init_vals = [8.90749e-01, 1.38770e+00, -1.97452e+00,
        #              1.53386e+00, 1.71850e+00, -6.06313e-02]
        # Lukin:
        init_vals = [-0.1122*2*np.pi, 1.0431, 0.7318,
                     1.215, 0.288*2*np.pi, 1.62]
        print(init_vals)
...
#%%
## 注意zihan的代码和lukin的公式相比，phi0是反着的，此外A取lukin的负号看起来效果更好一些
## 注意更改了光强配比的话，失谐需要更改，来让系统共振

temp = 30*10**(-6)
cz_test = CZ(mid_state_decay=1, ryd_decay=1, ryd_garbage=1, temp=temp)
# ff = []
# for i in np.linspace(0.27,0.3,11):
#     f=cz_test._obj_fidel_bell_mod()([-0.1122*2*np.pi, 1.0431, 0.7318,
#                      1.215, i*2*np.pi, 1.62])
#     ff.append(f)
#     print(i,f)
cz_test._obj_fidel_bell_mod()([-0.1122*2*np.pi, 1.0431, 0.7318,
                     1.215, 0.288*2*np.pi, 1.62])
cz_test._obj_fidel_bell_mod()([-6.722212553965307702e-01, 1.057168710533928468e+00, 3.548060888572284188e-01,
                                1.312053006755440787e+00, 2.732271089746659776e+00, 1.550465810832863056e+00 ])
...
#%%
fidel_list = cz_test.Fidelity_bell_mod_thermal(-6.722212553965307702e-01,
                1.057168710533928468e+00*cz_test.rabi_eff,
                3.548060888572284188e-01, 1.550465810832863056e+00*cz_test.rabi_eff,
                1.312053006755440787e+00*cz_test.time_scale,
                2.732271089746659776e+00,(2*2.732271089746659776e+00)+np.pi)
...
for index in range(n_shots):
    ...
    phase_0 = 0
    phase_1 = 2.732271089746659776e+00
    phase_2 = (2*2.732271089746659776e+00)+np.pi
    pauli_two = PauliTwo(phase_0, phase_1, phase_2)
    state_init = cz_test._state_embed(7,pauli_two.pp_state())
    all_shit_together = cz_test.Diagnosis_run(tq_ham_const_therm, 
                            -6.722212553965307702e-01, 1.057168710533928468e+00*cz_test.rabi_eff,
                            3.548060888572284188e-01, 1.550465810832863056e+00*cz_test.rabi_eff,
                            1.312053006755440787e+00*cz_test.time_scale,
                            state_init)
